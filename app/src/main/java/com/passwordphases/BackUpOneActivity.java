...
//        doGetWallet();
    ...
    /*public void doGetWallet() {
        if (AppGlobal.isNetwork(BackUpOneActivity.this)) {

            AppGlobal.showProgressDialog(BackUpOneActivity.this);
            Map<String, String> optioMap = new HashMap<>();
            optioMap.put("RegisterId", AppGlobal.getStringPreference(BackUpOneActivity.this, WsConstant.SP_REGISTER_ID));

            new RestClient(BackUpOneActivity.this).getInstance().get().WalletAmount(optioMap)
                    .enqueue(new Callback<WalletListResponse>() {
                        @Override
                        public void onResponse(Call<WalletListResponse> call, Response<WalletListResponse> response) {
                            AppGlobal.hideProgressDialog(BackUpOneActivity.this);
                            if (response.body() != null) {
                                if (response.body().getSuccess() == 1) {


                                    walletId = response.body().getInfo().get(response.body().getInfo().size() - 1).getWalletId();
                                    String[] splited = response.body().getInfo().get(response.body().getInfo().size() - 1).getWalletKey().split("\\s+");
                                    ...
                                    Toast.makeText(BackUpOneActivity.this, response.body().getMsg(), Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(BackUpOneActivity.this, "server is down please try again later!!", Toast.LENGTH_SHORT).show();
                            }
                        }
                        ...
                        public void onFailure(Call<WalletListResponse> call, Throwable t) {
                            try {
                                AppGlobal.hideProgressDialog(BackUpOneActivity.this);
                                Toast.makeText(BackUpOneActivity.this, getString(R.string.network_time_out_error), Toast.LENGTH_SHORT).show();
                            } catch (Exception e) {
            ...
            Toast.makeText(BackUpOneActivity.this, getString(R.string.network_error), Toast.LENGTH_SHORT).show();
        }
    }*/
